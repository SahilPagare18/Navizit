...
export default function MainTourist() {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await axios.get(`http://localhost:3004/test/reviews/${currentTouristLocation}`);
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      ...
      try {
        ...
        const response = await axios.post("http://localhost:3004/api/recommend", { email: user.email });
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      await axios.post("http://localhost:3004/test/reviews", reviewData); // Save review to backend
